import s from './CompatibilityBar.module.scss';
...
import CompatibilityPopupContent
  from "@/components/CompatibilityPage/CompatibilitySelect/CompatibilityPopupContent/CompatibilityPopupContent.jsx";

const CompatibilityBar = ({data, setData}) => {
    ...
    const handleClick = () => {
      if (data.status === 'completed') {
        navigate(`/compatibility/result/${data.related_object_id}`)
    ...
    return (
      <>
        <div className={s.waitingBar} onClick={handleClick}>
            ...
            <div className={s.flex}>
              ...
              <div>
                ...
                </div>
                {
                  ...
                  <Clock currentTimer={data.seconds_left} onEnd={endHandler} classname={s.clock}/>
                }
                {
                  data.status === 'completed' && <div className={s.ready}>Готово!</div>
                }
              </div>
            </div>
          </div>
          ...
          {
            data.status === 'completed' && <BsExclamationCircle className={s.icon}/>
          }

        </div>

        <WaitingPopup isOpen={isOpen} setIsOpen={setIsOpen} onUnderstood={understoodHandler}>         
          <CompatibilityPopupContent />
        </WaitingPopup>
      </>
    );
  }
